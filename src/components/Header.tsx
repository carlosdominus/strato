      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3 relative">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
          ...
          <button
            ...
            disabled={isSyncing}
            className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-[#11310C] hover:bg-[#1A4713] text-[#FAFBF6] text-xs font-bold cursor-pointer transition-all shadow-xs disabled:opacity-50"
            title="Atualizar dados da planilha do Google"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-[#C4C240] ${isSyncing ? 'animate-spin' : ''}`} />
            <span className="hidden sm:inline">{isSyncing ? 'Atualizando...' : 'Atualizar'}</span>
          </button>

          {/* Google User Connection Pill */}
          {googleUser ? (
            <div
              onClick={() => onNavigateToTab('configuracoes')}
              className="flex items-center gap-2 px-2.5 py-1.5 rounded-2xl bg-white/90 hover:bg-white border border-[#11310C]/15 cursor-pointer transition-all shadow-xs"
            ...
            >
              {googleUser.photoURL ? (
                <img
                  ...
                  className="w-5 h-5 rounded-full object-cover border border-[#C4C240]"
                />
              ) : (
                <div className="w-5 h-5 rounded-full bg-[#11310C] text-[#C4C240] text-[10px] font-bold flex items-center justify-center">
                  {googleUser.email ? googleUser.email[0].toUpperCase() : 'G'}
                </div>
              )}
              <span className="text-[11px] font-extrabold text-[#11310C] hidden md:inline truncate max-w-[100px]">
            ...
            <button
              onClick={onGoogleLogin ? onGoogleLogin : () => onNavigateToTab('configuracoes')}
              disabled={isLoggingIn}
              className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-white/90 hover:bg-white border border-[#11310C]/15 text-[#11310C] text-xs font-bold cursor-pointer transition-all shadow-xs"
